...
import dev.glowdragon.strobo.configuration.GroupServiceConfig
import dev.glowdragon.strobo.device.Device
import dev.glowdragon.strobo.device.DeviceService
import dev.glowdragon.strobo.util.StringIdGenerator
...
class GroupService(
  ...
  fun createGroup(name: String, deviceIds: Array<String>): Group {
    ...
    groupInfo.id =
        StringIdGenerator.generateUniqueId(
            this.groupInfoRepository, name, this.groupServiceConfig.maxIdLength)
    groupInfo.name = name
