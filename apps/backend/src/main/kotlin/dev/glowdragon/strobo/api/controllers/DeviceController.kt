...
import dev.glowdragon.strobo.api.classes.ApiDevice
import dev.glowdragon.strobo.api.classes.ApiDeviceList
import dev.glowdragon.strobo.api.classes.RegisterDeviceParams
import dev.glowdragon.strobo.api.classes.UpdateDeviceParams
import dev.glowdragon.strobo.api.exceptions.NotFoundException
import dev.glowdragon.strobo.device.Device
import dev.glowdragon.strobo.device.DeviceRepository
...
class DeviceController(
  ...
  fun getDevices(
    ...
    if (inAGroup != null) {
      for (group in this.groupService.allGroups) {
        if (inAGroup == true) {
          // TODO: Implement
          // devices.filter { device -> group.devices.any { groupDevice -> groupDevice.info.id ==
          // device.info.id } }
        } else {
          devices.removeIf { device ->
            group.devices.any { groupDevice -> groupDevice.entity.id == device.entity.id }
          }
        }
      }
    }
  ...
  fun getDevice(@PathVariable id: String): ApiDevice {
    val device =
        deviceService.allDevices.firstOrNull { device -> device.id == id }
  ...
  fun registerDevice(@PathVariable id: String, @RequestBody deviceParams: RegisterDeviceParams) {
    val device = this.deviceService.getDevice(id) ?: throw NotFoundException()
    ...
    var extraNumber = 2
    while (this.deviceService.allDevices.any { otherDevice ->
      otherDevice.entity.id != device.entity.id &&
          otherDevice.registered &&
          otherDevice.entity.name == name
    }) {
