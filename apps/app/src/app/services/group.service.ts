...
export class GroupService {
  ...
  getAllGroups(): Promise<Group[]> {
    return new Promise((resolve) => {
      this.http
        .get<GroupList>(environment.backend.serviceUrl + '/groups')
        .subscribe((result) => {
  ...
  getGroup(id: string): Promise<Group> {
    return new Promise((resolve) => {
      this.http
        .get<Group>(environment.backend.serviceUrl + '/groups/' + id)
        .subscribe((group) => {
          resolve(group);
        });
    });
  ...
  createGroup(name: string, deviceIds: string[]): Promise<null> {
    return new Promise((resolve) => {
      this.http
        .post(
          environment.backend.serviceUrl + '/groups',
          new CreateGroupParams(name, deviceIds)
        )
        .subscribe(() => {
          resolve(null);
        });
  ...
  updateGroup(group: Group): Promise<null> {
    return new Promise((resolve) => {
      ...
      this.http
        .put(
          environment.backend.serviceUrl + '/groups/' + group.id,
          new UpdateGroupParams(
            group.name,
            deviceIds,
            group.active,
            group.brightness,
  ...
  deleteGroup(id: string): Promise<null> {
    return new Promise((resolve) => {
      this.http
        .delete(environment.backend.serviceUrl + '/groups/' + id)
        .subscribe(() => {
          resolve(null);
        });
    });
