...
class CenterPulseAnimation(
  ...
  override fun updateProgress(args: UpdateProgressArgs) {
    ...
    this.drawPoint(
        args.leds, progress, args.totalLedCount / 2, 1, args.totalLedCount / 2 + 10, 10.0)
    this.drawPoint(
        args.leds, progress, args.totalLedCount / 2, -1, args.totalLedCount / 2 + 10, 10.0)
  }
  ...
  override fun newInstance(parameters: Map<String, Any>): Animation {
    return CenterPulseAnimation(
        this.id, this.name, this.rainbow) // parameters["rainbow"]!! as Boolean)
  }
}
