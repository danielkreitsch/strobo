...
fun main() {
  NadineDavidAnimation()
      .updateProgress(Animation.UpdateProgressArgs(LedArray(695), 0, 0, 120.0, 0, emptyMap()))
}

/** Back: 0 - 181 Left: 182 - 339 Front: 340 - 534 Right: 535 - 694 */
class NadineDavidAnimation : Animation("nadine-david", "Nadean & David") {
  ...
  private fun setColor(index: Int, color: Color, brightness: Double = 1.0) =
      leds[index].apply(
          (color.r * brightness).toInt(),
          (color.g * brightness).toInt(),
          (color.b * brightness).toInt())

  fun setWallColor(wall: Wall, index: Int, color: Color, brightness: Double = 1.0) =
      setColor(wall.index(index), color, brightness)

  fun multiplyBrightness(wall: Wall, index: Int, brightness: Double) =
      setColor(wall.index(index), leds[wall.index(index)], brightness)
  ...
  override fun updateProgress(args: UpdateProgressArgs) {
    ...
    (0 until Wall.RIGHT.size).forEach {
      setWallColor(Wall.RIGHT, it, Color(255, 0, 255), BRIGHTNESS)
    }
  ...
  private fun setGradient(
      ...
      endColor: Color,
      brightness: Double,
      gradientStartPercent: Float,
      gradientEndPercent: Float
  ) {
    val diff =
        Color(endColor.r - startColor.r, endColor.g - startColor.g, endColor.b - startColor.b)
    ...
    for (i in 0 until wall.size) {
      val color =
          when {
            ...
            i > gradientEnd ->
                Color(startColor.r + diff.r, startColor.g + diff.g, startColor.b + diff.b)
            else -> {
