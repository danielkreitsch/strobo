...
import com.danielkreitsch.strobo.backend.api.classes.*
...
import com.danielkreitsch.strobo.backend.group.GroupRepository
...
)
{
    ...
    {
        val group = this.groupService.allGroups.firstOrNull { group -> group.entity.id == id }
    ...
    fun updateGroup(@PathVariable id: String, @RequestBody params: UpdateGroupParams)
    {
        ...
        group.animation = this.animationService.allAnimations.firstOrNull { animation -> animation.id == params.animationId }?.newInstance(animationParams)
        for (device in group.devices)
        {
