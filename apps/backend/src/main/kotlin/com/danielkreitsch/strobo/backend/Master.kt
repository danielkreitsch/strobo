...
import com.danielkreitsch.strobo.backend.device.*
...
)
{
    ...
    fun checkHealth(deltaTime: Double)
    {
        ...
        {
            val healthSignalRecentlyReceived = device.lastHealthSignalAt != null && device.lastHealthSignalAt!! > System.currentTimeMillis() - 1500
    ...
    fun sendDeviceStates(time: Long, deltaTime: Double, devices: Array<Device>)
    {
        ...
        for (device in devices)
        {
            ...
            if (device.enabled && device.animation != null)
            {
                ...
                {
                    device.animation!!.updateProgress(Animation.UpdateProgressArgs(device.ledArray, totalLedCount, device.position, this.globalLightSettings.beatsPerTick, this.updateColorsTick, parameters))
                }
                else
                {
                    device.animation!!.updateProgress(Animation.UpdateProgressArgs(device.ledArray, device.ledCount, 0, this.globalLightSettings.beatsPerTick, this.updateColorsTick, parameters))
                }
            }

            if (!device.ledArray.contentEquals(device.previousLedArray) || (!device.enabled && device.numSequentialUpdatesWithoutColorChange < 100))
            {
                ...
                //println("Sent " + device.ledArray.colors[0].r + "," + device.ledArray.colors[0].g + "," + device.ledArray.colors[0].b + " to " + device.id)
