...
export class DevicePage implements OnInit {
  ...
  ngOnInit() {
    this.deviceOfflineImage =
      this.deviceOfflineImages[
        Math.floor(Math.random() * this.deviceOfflineImages.length)
      ];
  }
  ...
  fetchDevice() {
    this.deviceService
      .getDevice(this.route.snapshot.paramMap.get('id')!)
      .then((device) => {
        this.device = device;
      });
  }
