...
function KellyColorPicker(ca) {
  function T() {
    ...
    w.draw = function () {
      ...
      if (this.imageData[B])
        ...
        35 > this.size
          ? (b = a = v.width / 2)
          : ((a = v.width / 2 - this.lineWidth),
            f.arc(this.size / 2, this.size / 2, a, 0, 2 * I),
            ...
            f.closePath(),
            f.beginPath(),
            f.arc(this.size / 2, this.size / 2, b, 0, 2 * I),
            (f.strokeStyle = 'rgba(0, 0, 0, 0.4)'),
            (f.lineWidth = this.lineWidth),
            f.stroke(),
            f.closePath());
    ...
    w.isDotIn = function (a) {
      return a.x >= this.position.x &&
        a.x <= this.position.x + this.size &&
        a.y >= this.position.y &&
        a.y <= this.position.y + this.size
        ? !0
        : !1;
    };
  }
  function W(a, b, c) {
    ...
    d.updateSize = function () {
      ...
      'left' == this.align
        ? (this.position = { x: 0, y: u - this.width })
        : 'right' == this.align &&
          (this.position = { x: u - this.width, y: u - this.width });
    };
    d.calcS = function (a) {
      return (
        Math.abs(
          (a[1].x - a[0].x) * (a[2].y - a[0].y) -
    ...
    d.isDotIn = function (a) {
      var b = [];
      'left' == this.align
        ? ((b[0] = { x: this.position.x, y: this.position.y }),
          (b[1] = { x: this.position.x, y: this.position.y + this.width }),
          (b[2] = {
            x: this.position.x + this.width,
            y: this.position.y + this.width,
          }))
        : ((b[0] = { x: this.position.x + this.width, y: this.position.y }),
          (b[1] = { x: b[0].x, y: b[0].y + this.width }),
          (b[2] = { x: b[0].x - this.width, y: this.position.y + this.width }));
    ...
    d.draw = function () {
      ...
      f.beginPath();
      'left' == this.align &&
        (f.moveTo(this.lineWidth / 2, this.width - this.lineWidth),
      ...
      this.selected &&
        ...
        (f.lineWidth = this.lineWidth),
        'left' == this.align &&
          (f.moveTo(this.selectSize, this.width - this.selectSize),
          f.lineTo(
            this.width - 2 * this.selectSize,
            this.width - this.selectSize
          ),
          f.lineTo(this.selectSize, 2 * this.selectSize),
          f.lineTo(this.selectSize, this.width - this.selectSize)),
        'right' == this.align &&
          (f.moveTo(2 * this.selectSize, this.width - this.selectSize),
      ...
      if (((a = a.trim(a)), -1 == a.indexOf('('))) {
        if (
          ...
          if (3 <= a.length && 4 >= a.length)
            for (var d = '', e = 0; e < a.length; e++) d += a[e] + a[e];
      ...
      'object' == typeof a &&
        ...
        'undefined' != typeof a.b
          ...
          : 'undefined' != typeof a.h &&
            ...
            ((a.h = parseFloat(a.h) / 360),
            (a.s = parseFloat(a.s) / 100),
            (a.l = parseFloat(a.l) / 100),
            0 <= a.h && 1 >= a.s && 1 >= a.l && (d = U(O(a.h, a.s, a.l)))),
        'undefined' != typeof a.a && (c = a.a));
    if (!1 === d && b) return !1;
    !1 === d && (d = '000000');
    '#' != d.charAt(0)
      ? ((d = d.substr(0, 6)), (d = '#' + d))
  ...
  function ga() {
    ...
    a.draw = function () {
      this.imageData ||
        (this.imageData = e.createImageData(this.size, this.size));
      for (
        var a = 0, c = this.path[0].x, d = this.path[0].y, f = 0;
        f < this.size;
        f++
      )
        for (var g = 0; g < this.size; g++) {
          var h = this.dotToSv({ x: g + c, y: f + d }),
            h = O(l.h, h.s, h.v);
      ...
      for (a = 0; a <= this.path.length - 1; ++a)
        0 == a
          ? e.moveTo(this.path[a].x, this.path[a].y)
          : e.lineTo(this.path[a].x, this.path[a].y);
      e.stroke();
      e.closePath();
    };
    a.updateSize = function () {
      this.size = Math.floor(
        (2 * h.innerRadius - 2 * x.paddingX - 2 * this.padding) / Math.sqrt(2)
      );
      this.path = [];
      this.path[0] = { x: (this.size / 2) * -1, y: (this.size / 2) * -1 };
    ...
    a.isDotIn = function (a) {
      return a.x < this.path[0].x ||
        a.x > this.path[0].x + this.size ||
        a.y < this.path[0].y ||
        a.y > this.path[0].y + this.size
        ? !1
        : !0;
    };
    return (V.quad = a);
  ...
  function ha() {
    ...
    a.svToDot = function (a) {
      var c = 0.02;
      150 > u ? (c = 0.07) : 100 > u && (c = 0.16);
      for (var d = 0; d < this.size; d++)
        for (var e = 0; e < this.size; e++) {
          var f = { x: this.path[1].x + e, y: this.path[1].y + d };
          if (y.isDotIn(f)) {
    ...
    a.draw = function () {
      this.imageData ||
        (this.imageData = f.createImageData(this.size, this.size));
      v.width = this.size;
      v.height = this.size;
      for (
        var a = this.path[1].x, c = this.path[1].y, d = 0, g = 0;
        g < this.size;
        g++
      )
        for (var h = 0; h < this.size; h++) {
          var k = { x: this.path[1].x + h, y: this.path[1].y + g };
          y.isDotIn(k)
            ? ((k = this.dotToSv(k)),
              (k = O(l.h, k.s, k.v)),
              ...
              (this.imageData.data[d + 3] = 255))
            : ((this.imageData.data[d + 0] = 0),
    ...
    a.dotToSv = function (a) {
      var c = this.vol,
        d =
          ((a.x - c[0].x) * (c[1].x - c[0].x) +
            (a.y - c[0].y) * (c[1].y - c[0].y)) /
          ((c[0].x - c[1].x) * (c[0].x - c[1].x) +
            (c[0].y - c[1].y) * (c[0].y - c[1].y));
      0 > d && (d = 0);
      1 < d && (d = 1);
      var e = this.vol[0],
        c = Math.sqrt(
          Math.pow(c[0].x + d * (c[1].x - c[0].x) - e.x, 2) +
            Math.pow(c[0].y + d * (c[1].y - c[0].y) - e.y, 2)
        );
      1 > c && (c = Math.floor(c));
      c > this.h - 1 && (c = this.h);
      c /= this.h;
    ...
    a.isDotIn = function (a) {
      ...
      var c = this.calcS(a);
      a[1] = { x: this.path[2].x, y: this.path[2].y };
      c += this.calcS(a);
      a[0] = { x: this.path[1].x, y: this.path[1].y };
      c += this.calcS(a);
    ...
    a.updateSize = function () {
      ...
      this.path = [];
      this.path[0] = { x: this.outerRadius, y: 0 };
      this.path[1] = { x: this.path[0].x - a, y: (this.size / 2) * -1 };
      this.path[2] = { x: this.path[1].x, y: this.size / 2 };
      this.path[3] = { x: this.path[0].x, y: this.path[0].y };
      for (a = 0; a <= this.path.length - 1; ++a)
        (this.path[a].x += h.pos.x), (this.path[a].y += h.pos.y);
      this.vol = [];
      this.s = this.calcS(this.path);
      this.sOnTop
        ? ((a = ja(this.path[0], this.path[2])),
          (this.vol[0] = { x: this.path[1].x, y: this.path[1].y }),
          (this.vol[1] = { x: a.x, y: a.y }),
          (this.sSide = this.path[1]))
        : ((a = ja(this.path[0], this.path[1])),
  ...
  function z(a, b, c, d) {
    ...
    P[d + b] = c;
    a.addEventListener
      ? a.addEventListener(b, P[d + b])
      : a.attachEvent('on' + b, P[d + b]);
    return !0;
  }
  function q(a, b, c) {
    'object' !== typeof a && (a = document.getElementById(a));
    ...
    a.removeEventListener
      ? a.removeEventListener(b, P[c + b])
      : a.detachEvent('on' + b, P[c + b]);
    P[c + b] = null;
    return !0;
  }
  function O(a, b, c) {
  ...
  function Z(a) {
    ...
    return { r: a >> 16, g: (a >> 8) & 255, b: a & 255 };
  }
  function U(a) {
    var b = function (a) {
      a = a.toString(16);
  ...
  function da() {
    ...
    x.path = [
      { x: h.innerRadius - x.paddingX, y: -1 * x.height },
      { x: h.outerRadius + x.paddingX, y: -1 * x.height },
      { x: h.outerRadius + x.paddingX, y: x.height },
      { x: h.innerRadius - x.paddingX, y: x.height },
      { x: h.innerRadius - x.paddingX, y: -1 * (x.height + x.lineWeight / 2) },
    ];
    var a = u;
    E && (a += t.width + 2 * t.padding);
    'canvas' != F.tagName.toLowerCase() &&
      ((F.style.width = a + 'px'), (F.style.height = u + 'px'));
    n.width = a;
    n.height = u;
    Q != n && ((n.style.width = a + 'px'), (n.style.height = u + 'px'));
    for (a = 0; a <= m.length - 1; ++a) m[a].updateSize();
    w &&
      ((w.imageData.triangle = null),
  ...
  function ta() {
    if (!F) return !1;
    'CANVAS' != F.tagName
      ? ((n = document.createElement('CANVAS')), F.appendChild(n))
      : (n = F);
    ...
    return n.getContext && n.getContext('2d')
      ? ((e = n.getContext('2d')), (f = v.getContext('2d')), !0)
      : !1;
  }
  function ba() {
    z(
  ...
  function R(a) {
    ...
    'touchend' == a.type
      ? ((b = a.changedTouches[0].clientX + c),
        (a = a.changedTouches[0].clientY + d))
      : 'touchmove' == a.type || a.touches
      ? ((b = a.touches[0].clientX + c), (a = a.touches[0].clientY + d))
      : ((b = a.clientX + c), (a = a.clientY + d));
  ...
  function ea() {
    for (var a = 0; a <= m.length - 1; ++a) m[a].selected && (m[a].color = D);
  }
  function qa() {
    if (m.length) for (var a = 0; a <= m.length - 1; ++a) m[a].draw();
  }
  function ua() {
    if (!e) return !1;
  ...
  function M() {
    ...
    E &&
      (e.beginPath(),
      e.rect(t.pos.x - 2, t.padding + t.height * (1 - G) - 1, t.width + 4, 2),
    ...
    e.closePath();
    e.strokeStyle =
      0.5 < l.v && 0.5 > l.s ? 'rgba(0, 0, 0, 1)' : 'rgba(255, 255, 255, 1)';
    e.beginPath();
    e.lineWidth = 2;
  ...
  h.pos;
  h.draw = function () {
    if (this.imageData) e.putImageData(this.imageData, 0, 0);
    else {
  ...
  h.isDotIn = function (a) {
    return Math.pow(this.pos.x - a.x, 2) + Math.pow(this.pos.y - a.y, 2) <
      Math.pow(this.outerRadius, 2) &&
      Math.pow(this.pos.x - a.x, 2) + Math.pow(this.pos.y - a.y, 2) >
        Math.pow(this.innerRadius, 2)
      ? !0
      : !1;
  };
  var x = { lineWeight: 2, height: 4, paddingX: 2 };
  x.path;
  var t = {
    width: 18,
    padding: 4,
  ...
  t.updateSize = function () {
    this.pos = { x: u + t.padding, y: t.padding };
  ...
  t.alphaToDot = function (a) {
    return { x: 0, y: this.height - this.height * a };
  };
  t.limitDotPosition = function (a) {
    a = a.y;
    a < this.pos.y && (a = this.pos.y);
    a > this.pos.y + this.height && (a = this.pos.y + this.height);
    return { x: this.pos.x, y: a };
  };
  t.isDotIn = function (a) {
    return a.x < this.pos.x ||
      a.x > this.pos.x + t.width ||
      a.y < this.pos.y ||
      a.y > this.pos.y + this.height
      ? !1
      : !0;
  };
  var C = { svCursorData: null, stCursor: null, curType: 0, size: 16 };
  C.cEl = document.body;
  C.initSvCursor = function () {
    ...
    if (this.svCursorData)
      return (this.cEl.style.cursor = this.svCursorData), !0;
    if (!v) return !1;
    var a = this.size + 2;
    v.width = a;
    v.height = a;
  ...
  C.updateCursor = function (a) {
    sa &&
      (KellyColorPicker.cursorLock ||
        (y.isDotIn(a) ? C.initSvCursor() : C.initStandartCursor()));
  };
  this.popUpClose = function (a) {
    if (!1 !== p.tag) {
      if (a && (a.target == r || a.target == n || a.target == p.tag)) return !1;
      if (!k.popupclose || k.popupclose(g, a))
  ...
  this.popUpShow = function (a) {
    if (!1 !== p.tag && (!k.popupshow || k.popupshow(g, a))) {
      KellyColorPicker.popupEventsInclude ||
        (z(
          document,
          'click',
          function (a) {
            return KellyColorPicker.activePopUp
              ? KellyColorPicker.activePopUp.popUpClose(a)
              : !1;
          },
          'popup_close_'
        ),
      ...
      KellyColorPicker.activePopUp &&
        KellyColorPicker.activePopUp.popUpClose(!1);
      a = g.getCanvas().width;
      var b = g.getAlphaFig();
  ...
  this.mouseDownEvent = function (a) {
    ...
    else if (w && w.isDotIn(d)) g.setMethod();
    else if (m.length)
  ...
  this.getColorSaver = function (a) {
    for (var b = 0; b <= m.length - 1; ++b)
      if ((!a && m[b].selected) || m[b].align == a)
        return (
          (m[b].rgb = Z(m[b].color)),
          (m[b].hsv = ka(m[b].rgb.r, m[b].rgb.g, m[b].rgb.b)),
          m[b]
        );
  };
  ...
  this.destroy = function () {
    if (!g) return !1;
    0 < C.curType &&
      ((KellyColorPicker.cursorLock = !1), C.initStandartCursor());
    A &&
      (q(document, 'mouseup', 'action_process_'),
      q(document, 'mousemove', 'action_process_'),
      q(document, 'touchend', 'action_process_'),
    ...
    r &&
      (q(r, 'click', 'input_edit_'),
      q(r, 'change', 'input_edit_'),
      q(r, 'keyup', 'input_edit_'),
      q(r, 'keypress', 'input_edit_'));
    KellyColorPicker.popupEventsInclude &&
      P.popup_close_click &&
      (KellyColorPicker.activePopUp &&
        KellyColorPicker.activePopUp.popUpClose(!1),
  ...
  (function (a) {
    ...
    a.input && 'object' !== typeof a.input
      ? ((a.input = document.getElementById(a.input)), (r = a.input))
      : 'object' === typeof a.input && (r = a.input);
    ...
    a.place &&
      ...
      ((c = a.place), (a.place = document.getElementById(a.place)));
    a.place
      ? (F = a.place)
      : r
      ...
      : (b += '| "place" (' + c + ') not not found');
    c = !1;
    a.color ? (c = N(a.color)) : r && r.value && (c = N(r.value));
    c && ((D = c.h), E && (G = c.a));
    !a.method ||
      ('triangle' != a.method && 'quad' != a.method) ||
      (B = a.method);
    ta() || (b += ' | cant init canvas context');
    a.resizeWith &&
      ('object' !== typeof a.resizeWith &&
        'boolean' !== typeof a.resizeWith &&
        (a.resizeWith = document.getElementById(a.resizeWith)),
      (Q = !0 === a.resizeWith ? n : a.resizeWith),
      a.resizeSide && (aa = a.resizeSide),
      Q &&
        (S(Q) && (a.size = S(Q)),
        z(
          window,
    ...
    b
      ? 'undefined' !== typeof console && console.log('KellyColorPicker : ' + b)
      : ('quad' == B && (y = ga()),
        'triangle' == B && (y = ha()),
        r &&
          ((b = function (a) {
          ...
          z(r, 'click', b, 'input_edit_'),
          z(r, 'change', b, 'input_edit_'),
          z(r, 'keyup', b, 'input_edit_'),
          z(r, 'keypress', b, 'input_edit_')),
        a.colorSaver && (W('left', !0), W('right')),
        a.methodSwitch && T(),
        ba(),
        da(),
        g.setColorByHex(!1));
...
KellyColorPicker.attachToInputByClass = function (ca, T) {
  for (
    var W = [], S = document.getElementsByClassName(ca), N = 0;
    N < S.length;
    N++
  )
    T ? (T.input = S[N]) : (T = { input: S[N], size: 150 }),
